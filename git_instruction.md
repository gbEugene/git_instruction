...
## `11.` Работа с удалённым репозиторием.

Чтобы начать работать с репозиториями в облаке, необходимо завести аккаунт на https://github.com

Теперь, авторизовавшись на сайте можно приступать к работе. 
...
Далее связываем наш удалённый и локальный репозторий командой 
```
git remote add origin <ссылка на наш удалённый репозиторий>
```
Теперь мы можем создать файл или там или там, изменять их, и после, уже через терминал на компьютере либо загрузить актульную версию репозитория 
```
git pull
```
...
либо выгрузить её на сайт

*но для этого потребуется авторизоваться* 
```
git push
```
![git_push](git_push.JPG)

Так же мы можем скопировать интересующий нас репозиторий, то есть создать клон
```
git clone <адрес того репозитория, который надо скачать>
```